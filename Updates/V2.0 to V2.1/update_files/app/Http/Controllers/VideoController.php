...
class VideoController extends Controller
{
    ...
    public function search(Request $request)
    {
       ...
       if ($validator->fails()) {
            ...
            $data['videos'] = Video::where('status',1)->where('published',1)->where('title','like','%'.$keyword.'%')->orderBy('views','DESC')->paginate(config('app.videos_per_page'));
    ...
    {
        $data['videos'] = Video::where('user_id',Auth::user()->id)->orderBy('id','DESC')->get();
